...
- The server provides validated map rooms, snapshots, rate-limited chat, idle cleanup, health reporting, automated tests, and durable identity storage in a two-instance PostgreSQL cluster.
...
The native gpSP runtime now boots reliably on physical hardware. During bring-up we fixed the CIA/Luma SVC bootstrap, a null libretro capability-probe dereference, a double-VBlank 30 FPS limiter, the padded/tiled GPU upload used by the 400x240 top display, ctrulib's unreliable nonblocking `select()` completion path, a 65536/32768 Hz NDSP mismatch, and the gpSP RAM binding used for Emerald state. An Old 3DS XL now runs at 60 FPS with smooth audio and publishes live map/tile position updates. The 0.4.0 protocol-v2 build has passed Azahar and public-WSS automation; physical-WSS acceptance remains a tester gate.
...
The server is deliberately content-agnostic. The client owns local simulation and sends a compact presence state; the server validates sequence, position, map membership, and device authentication, then broadcasts map snapshots. PostgreSQL persists identity and community metadata, never ROM/save/party/inventory data or routine chat. Forum, consented leaderboards, and the separately gated link-multiplayer experiment remain later milestones; see [COMMUNITY_PLATFORM_PLAN.md](COMMUNITY_PLATFORM_PLAN.md).
