...
- The server provides validated map rooms, snapshots, rate-limited chat, idle cleanup, health reporting, automated tests, durable identity storage, and a paired community forum in a two-instance PostgreSQL cluster.
...
The native gpSP runtime now boots reliably on physical hardware. During bring-up we fixed the CIA/Luma SVC bootstrap, a null libretro capability-probe dereference, a double-VBlank 30 FPS limiter, the padded/tiled GPU upload used by the 400x240 top display, ctrulib's unreliable nonblocking `select()` completion path, a 65536/32768 Hz NDSP mismatch, and the gpSP RAM binding used for Emerald state. An Old 3DS XL now runs at 60 FPS with smooth audio and publishes live map/tile position updates. The 0.5.0 build adds production-tested WSS clock handling and browser pairing for the integrated community; physical acceptance remains a tester gate.
...
Emerald uses the normal GBA controls. `X` toggles online/offline, `Y` switches the lower screen between the online and party pages, tapping the trainer profile card opens browser-pairing entry, tapping the chat panel opens same-map chat, and the four bottom touch zones send Wave, Battle, Trade, or GG emotes. Gameplay is permanently locked to the native 400x240 top screen. When offline, the dashboard shows the configured endpoint and latest socket error number. HOME exits. Online failure never stops local gameplay. Set `page=party` in `online.cfg` to open on the party page.

See [TESTING.md](TESTING.md) for emulator and physical-3DS smoke tests.

## Architecture

The server is deliberately content-agnostic. The client owns local simulation and sends a compact presence state; the server validates sequence, position, map membership, and device authentication, then broadcasts map snapshots. PostgreSQL persists identity and community metadata, never ROM/save/party/inventory data or routine chat. The paired forum is live at [emeraldonline3ds.com/community](https://emeraldonline3ds.com/community); consented leaderboards and the separately gated link-multiplayer experiment remain later milestones. See [COMMUNITY_PLATFORM_PLAN.md](COMMUNITY_PLATFORM_PLAN.md).
