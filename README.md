...
The native gpSP runtime now boots reliably on physical hardware. During bring-up we fixed the CIA/Luma SVC bootstrap, a null libretro capability-probe dereference, a double-VBlank 30 FPS limiter, the padded/tiled GPU upload used by the 400x240 top display, ctrulib's unreliable nonblocking `select()` completion path, a 65536/32768 Hz NDSP mismatch, and the gpSP RAM binding used for Emerald state. An Old 3DS XL now runs at 60 FPS with smooth audio and publishes live map/tile position updates. Version 0.7.1 retains the Gate 3 profiles, privacy controls, leaderboards, and opt-in Gate 4 Serial-Poke feasibility transport, and gives failed connections a dedicated readable bottom-screen panel with the socket error, named protocol stage, TLS result, certificate flags, clock skew, and SD log path. The experimental link room is disabled by default, accepts exactly two authenticated clients, and refuses to start the gpSP netpacket session unless a timestamped save backup has been flushed and fsynced. Public invitations, battle/trade rankings, and completion claims remain disabled pending Cable Club and physical acceptance.
