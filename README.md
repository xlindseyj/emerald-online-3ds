...
- The native lower screen provides online status, a paged global online-user roster with coordinates, switchable session-only Map and Global Chat with UTC timestamps and tap-to-read messages, touch emotes, a local-only party summary, and an explicit-consent statistics page.
