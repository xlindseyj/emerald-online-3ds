...
The native gpSP runtime now boots reliably on physical hardware. During bring-up we fixed the CIA/Luma SVC bootstrap, a null libretro capability-probe dereference, a double-VBlank 30 FPS limiter, the padded/tiled GPU upload used by the 400x240 top display, ctrulib's unreliable nonblocking `select()` completion path, a 65536/32768 Hz NDSP mismatch, and the gpSP RAM binding used for Emerald state. An Old 3DS XL now runs at 60 FPS with smooth audio and publishes live map/tile position updates. Version 0.8.2 includes the local-only Bag and Map/Radar pages, Gate 3 profiles, privacy controls, leaderboards, readable network diagnostics, the opt-in Gate 4 Serial-Poke feasibility transport, a validated progressed-save handoff, and Azahar interpreter/network stability fixes. The Bag reads item names from the user's private validated ROM and decrypts local quantities in memory; none of that data is uploaded or packaged. The experimental link room is disabled by default, accepts exactly two authenticated clients, and refuses to start the gpSP netpacket session unless a timestamped save backup has been flushed and fsynced. The exact release now reaches its isolated production-WSS link room with the progressed save in Azahar, but public invitations, battle/trade rankings, and completion claims remain disabled pending real Cable Club and physical acceptance.
