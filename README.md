...
An iOS 15+ sideload preview is also available. It uses a dedicated Capacitor
launcher and native libretro host to run the same ROM-free 3DSX through a pinned
Azahar core, accepts only the supported user-supplied Emerald dump, and exposes
no general-purpose game library. Add
`https://emeraldonline3ds.com/source.json` to SideStore or use the iOS tab on
the installer site. The preview is a compiled ARM64 app, but physical iPhone
installation, gameplay, and performance acceptance remain open. See
[IOS_SIDELOAD_HANDOFF.md](IOS_SIDELOAD_HANDOFF.md) for the complete architecture,
build, privacy, distribution, and test record.
