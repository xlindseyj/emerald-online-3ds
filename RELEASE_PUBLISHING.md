...
The initial backfill contains factual posts for 0.3.2, 0.5.0, 0.6.1, 0.7.1, and 0.8.0. Version 0.8.1 adds production observability and bounded-load evidence; version 0.8.2 adds the validated-save Gate 4 handoff and Azahar interpreter/network stability fixes; version 0.8.3 restricts remote-trainer rendering to Emerald's verified overworld context; version 0.8.4 adds the read-only global trainer roster, timestamped current-map Map Chat page, authentic lower-screen media, and reduced save-stat polling. Historical builds are clearly marked superseded and are not offered as current downloads.
...
The v0.8.3 hotfix rollout uses multi-architecture image `sha256:3f185c8c132cfe3481dd9ec13b2e286862a8b09a2eb654b678d0127a3c3fb1c8`. Both init containers exited 0, both application containers are Ready with zero restarts, and the publisher reported eight idempotent release topics, one known issue, and nine maintained community pages with v0.8.3 current. The homepage, public status API, forums, CIA, 3DSX, and source archive were verified live; all four monitored services report operational and all live artifact hashes match the release manifest.

The v0.8.4 rollout uses multi-architecture image `sha256:b0b501e92a3a106fc29eeedd6a5c8829ae3c3fa1d801e22416e22393b9f26668`. Both init containers exited 0, both application containers are Ready with zero restarts, and anonymous reads show v0.8.4 as the sole pinned current release plus all nine maintained community pages. The installation, emulator, and live-status guides are public and official; the emulator guide and homepage serve both authentic lower-screen captures. All four public status checks report Operational, the CIA/3DSX/source hashes match the live manifest, and a temporary two-client production-WSS check confirmed the global cross-map roster, snapshot isolation, coordinate values, and server chat timestamp before disconnecting.
