...
static void ctr_init_stage(const char *stage, int32_t result)
{
   FILE *file = fopen("sdmc:/3ds/emerald-online-3ds/gpsp-debug.log", "a");
   if (!file)
      return;
   fprintf(file, "core %s result=%08lx\n", stage, (unsigned long)result);
   fclose(file);
}
...
void retro_init(void)
{
   ...
   if (__ctr_svchax && !translation_caches_inited)
   {
      ...
      int32_t duplicate_result;
      int32_t rom_map_result;
      int32_t ram_map_result;
      ctr_init_stage("dynarec-begin", 0);
      check_rosalina();
      ctr_init_stage("rosalina-checked", 0);
      ...
      ctr_init_stage("buffers-allocated", rom_translation_cache_ptr && ram_translation_cache_ptr ? 0 : -1);

      if (!rom_translation_cache_ptr || !ram_translation_cache_ptr)
      {
         free(rom_translation_cache_ptr);
         free(ram_translation_cache_ptr);
         rom_translation_cache_ptr = NULL;
         ram_translation_cache_ptr = NULL;
         __ctr_svchax = 0;
      }

      if (__ctr_svchax)
      {
         duplicate_result = svcDuplicateHandle(&currentHandle, 0xFFFF8001);
         ctr_init_stage("handle-duplicated", duplicate_result);
         rom_map_result = duplicate_result < 0 ? duplicate_result :
         ...
         ctr_init_stage("rom-cache-mapped", rom_map_result);
         ram_map_result = (duplicate_result < 0 || rom_map_result < 0) ? -1 :
            svcControlProcessMemory(currentHandle,
                                    ram_translation_cache, ram_translation_cache_ptr,
                                    RAM_TRANSLATION_CACHE_SIZE, MEMOP_MAP, 0b111);
         ctr_init_stage("ram-cache-mapped", ram_map_result);
         if (duplicate_result >= 0)
            svcCloseHandle(currentHandle);

         if (duplicate_result >= 0 && rom_map_result >= 0 && ram_map_result >= 0)
         {
            rom_translation_ptr = rom_translation_cache;
            ram_translation_ptr = ram_translation_cache;
            ctr_init_stage("cache-flush-begin", 0);
            ctr_flush_invalidate_cache();
            ctr_init_stage("cache-flush-complete", 0);
            translation_caches_inited = 1;
            ctr_init_stage("translation-caches-ready", 0);
         }
         else
         {
            __ctr_svchax = 0;
            ctr_init_stage("dynarec-disabled", -1);
         }
      }
   }
  #elif defined(VITA)
   ...
   ctr_init_stage("gamepak-buffer-begin", 0);
   init_gamepak_buffer();
   ctr_init_stage("gamepak-buffer-ready", 0);
   ctr_init_stage("sound-init-begin", 0);
   init_sound();
   ctr_init_stage("sound-init-ready", 0);

   if(!gba_screen_pixels)
#ifdef _3DS
   {
      ctr_init_stage("framebuffer-alloc-begin", 0);
      gba_screen_pixels = (uint16_t*)linearMemAlign(GBA_SCREEN_BUFFER_SIZE, 128);
      ctr_init_stage("framebuffer-alloc-complete", gba_screen_pixels ? 0 : -1);
   }
#else
   ...
   ctr_init_stage("environment-probes-begin", 0);
   ...
   ctr_init_stage("environment-probes-ready", 0);
