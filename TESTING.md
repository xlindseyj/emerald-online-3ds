...
- Automated verification: all 60 tests pass with zero skips after migrations 001-006 against disposable PostgreSQL 16, including the fail-closed stationary test-peer regression and safe target-map selection. The public build, catalog validation, and release audit all passed.
...
- Physical status: the v0.8.4 CIA and 3DSX are installed/staged and hash-verified on the Old 3DS with all private files unchanged. The user confirmed Global Online Users, coordinates, read-only rows, and multi-page touch navigation. Map Chat, audio, HOME lifecycle, and FPS remeasurement remain pending. A checksum-valid cloned v0.8.4 Azahar peer now hosts the same private Cable Club room; the physical client must reconnect before both backup-safe markers and remaining Gate 4 acceptance can be checked.
...
2. Confirm the production pod annotations advertise port 3211 and `/metrics`, while NetworkPolicy permits that port only from the Prometheus pod in `<backup-namespace>` and node health probes.
