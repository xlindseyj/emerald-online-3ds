...
The temporary `Brendan` peer privately discovers the first positioned trainer, trails only exact tiles that trainer has already occupied, sends one greeting, waves, and disconnects after the requested duration. It never invents adjacent coordinates because the server intentionally has no private ROM-derived map collision data; after a warp or skipped update it teleports to the exact observed tile instead of animating through unknown terrain. The utility does not expose the private client-discovery endpoint publicly and never creates a database identity, score, forum account, ROM, or save. Move several tiles and confirm the lower screen shows `Brendan` under Nearby and that its coordinates follow the proven path without entering trees, walls, water, ledges, or other blocked tiles. The current external overlay is composited after Emerald's completed framebuffer, so a trainer on a valid path can still appear in front of a sign or tree canopy that should visually occlude it; foreground/object-priority integration remains a separate roadmap item.
