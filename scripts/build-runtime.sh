...
if [[ -n "${link_room}" && ! "${link_room}" =~ ^[A-HJ-NP-Z2-9]{4}-[A-HJ-NP-Z2-9]{4}$ ]]; then echo 'LINK_ROOM must use XXXX-XXXX without 0, 1, I, or O' >&2; exit 1; fi
...
if [[ -n "${link_room}" ]]; then printf 'link_room=%s\n' "${link_room}" >> "${deploy_dir}/online.cfg"; fi
