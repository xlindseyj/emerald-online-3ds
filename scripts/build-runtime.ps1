param(
    ...
    [string]$ServerHost = 'pokemon-server.lws-workspace.com',
...
if (-not $SessionToken) {
    $existingConfig = Join-Path $deploy 'online.cfg'
    if (Test-Path -LiteralPath $existingConfig) {
        $match = [regex]::Match([System.IO.File]::ReadAllText($existingConfig), '(?m)^session=([0-9a-fA-F]{32})\r?$')
        if ($match.Success) { $SessionToken = $match.Groups[1].Value }
    }
    if (-not $SessionToken) { $SessionToken = [Guid]::NewGuid().ToString('N') }
}
if ($SessionToken -notmatch '^[0-9a-fA-F]{32}$') { throw 'SessionToken must be exactly 32 hexadecimal characters.' }
$SessionToken = $SessionToken.ToLowerInvariant()
...
docker run --rm -e DEVKITPRO=/opt/devkitpro -e DEVKITARM=/opt/devkitpro/devkitARM -e CTRULIB=/opt/devkitpro/libctru -v "${projectRoot}\third_party\gpsp:/src" -w /src devkitpro/devkitarm:latest make platform=ctr -j8
if ($LASTEXITCODE -ne 0) { throw 'gpSP 3DS dynarec core build failed.' }
docker run --rm -e DEVKITPRO=/opt/devkitpro -e DEVKITARM=/opt/devkitpro/devkitARM -e CTRULIB=/opt/devkitpro/libctru -v "${projectRoot}:/project" -w /project/gpsp-runtime devkitpro/devkitarm:latest make -j8
if ($LASTEXITCODE -ne 0) { throw 'Dedicated gpSP frontend build failed.' }
docker run --rm -v "${projectRoot}:/project" -w /project/generated/private-avatar-build devkitpro/devkitarm:latest tex3ds -i avatars.t3s -o avatars.t3x
if ($LASTEXITCODE -ne 0) { throw 'Private Emerald avatar atlas build failed.' }
docker run --rm -v "${projectRoot}:/project" -w /project mgba/3ds:latest sh -lc "/opt/devkitpro/devkitARM/bin/arm-none-eabi-strip gpsp-runtime/emerald-online-3ds.elf -o gpsp-runtime/emerald-online-3ds-stripped.elf && /opt/devkitpro/tools/bin/bannertool makebanner -i assets/emerald-online-3ds-banner.png -a third_party/mgba/src/platform/3ds/bios.wav -o gpsp-runtime/emerald-online-3ds.bnr && /opt/devkitpro/tools/bin/makerom -f cia -o gpsp-runtime/emerald-online-3ds.cia -rsf third_party/mgba/src/platform/3ds/cia.rsf.in -target t -exefslogo -elf gpsp-runtime/emerald-online-3ds-stripped.elf -icon gpsp-runtime/emerald-online-3ds.smdh -banner gpsp-runtime/emerald-online-3ds.bnr -major 0 -minor 2 -micro 0"
...
$config = "server=$ServerHost`nport=$ServerPort`ntransport=$Transport`npath=$ServerPath`nname=$TrainerName`nsession=$SessionToken`n"
