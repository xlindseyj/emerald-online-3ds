...
for (const artifact of [ciaPath, threeDsxPath, sourcePath, checksumsPath]) {
...
:root{color-scheme:dark;font-family:ui-rounded,system-ui,sans-serif;background:#061b16;color:#effff8}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 12% 0,#176348,#061b16 52%);padding:32px 16px}main{width:min(980px,100%);margin:auto}.hero,.card{border:1px solid #65d6a255;border-radius:24px;background:#09251fe8;box-shadow:0 24px 80px #0007}.hero{padding:34px;margin-bottom:22px}.eyebrow{color:#71e6ad;font-weight:800;letter-spacing:.12em;text-transform:uppercase;font-size:.78rem}h1{font-size:clamp(2.2rem,7vw,4.8rem);line-height:.96;margin:.35em 0 .25em;color:#eafff5}h1 span{color:#6ce0a7}.lede{max-width:680px;color:#bfe9d5;font-size:1.08rem}.status{display:inline-flex;align-items:center;gap:9px;margin-top:12px;padding:8px 12px;border-radius:999px;background:#0e342a;color:#d9ffed}.dot{width:10px;height:10px;border-radius:50%;background:#f3c969;box-shadow:0 0 14px #f3c969}body.ready .dot{background:#58e49b;box-shadow:0 0 14px #58e49b}.grid{display:grid;grid-template-columns:minmax(250px,360px) 1fr;gap:22px}.card{padding:26px}.qr{padding:14px;background:white;border-radius:18px;line-height:0}.qr svg{width:100%;height:auto}code{word-break:break-all;color:#9de9c4}.button{display:inline-block;margin-top:10px;padding:13px 18px;border-radius:11px;background:#51d596;color:#052017;text-decoration:none;font-weight:800}.warning{color:#ffd68a}.endpoint{padding:13px;border-radius:12px;background:#061b16;border:1px solid #4e9f7a55}li{margin:.55em 0}@media(max-width:720px){.grid{grid-template-columns:1fr}.hero,.card{padding:22px}body{padding:18px 12px}}
</style></head><body><main><section class="hero"><div class="eyebrow">Public multiplayer presence service</div><h1>Emerald <span>Online</span> 3DS</h1><p class="lede">Play your own legally obtained Emerald copy on 3DS while seeing trainers on the same map, chatting, and sharing emotes. The ROM and save remain on your SD card.</p><div class="status"><span class="dot"></span><span id="status">Checking server…</span></div></section><div class="grid"><section class="card"><div class="qr">${qrSvg}</div></section><section class="card"><h2>Install with FBI</h2><ol><li>Put your own validated ROM and <code>online.cfg</code> on the SD card first.</li><li>Open FBI on the 3DS.</li><li>Choose <b>Remote Install</b>, then <b>Scan QR Code</b>.</li><li>Scan the code and confirm installation.</li></ol><a class="button" href="/emerald-online-3ds.cia">Download CIA</a><p><code>${ciaUrl}</code></p><p><a href="/emerald-online-3ds.3dsx">3DSX</a> · <a href="/source">Corresponding source</a> · <a href="/SHA256SUMS">SHA-256 checksums</a></p><h3>Game endpoint</h3><p class="endpoint"><code>${gamePublicUrl}</code></p><p class="warning"><b>Private ROM required:</b> the CIA contains no Pokemon ROM. Keep <code>emerald.gba</code>, saves, session token, and avatar atlas private at <code>sd:/3ds/emerald-online-3ds/</code>.</p></section></div></main><script>
