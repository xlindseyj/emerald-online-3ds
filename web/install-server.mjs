...
fetch('/api/status',{cache:'no-store'}).then(async response=>{const data=await response.json();if(!response.ok||!data.ok)throw new Error();document.body.classList.add('ready');const registered=data.registered!=null?' · '+data.registered+' registered':'';document.getElementById('status').textContent='Online · '+data.authenticated+' trainer'+(data.authenticated===1?'':'s')+' connected'+registered;}).catch(()=>{document.getElementById('status').textContent='Server temporarily unavailable';});
</script></body></html>`;
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/status') {
    try {
      ...
      res.writeHead(200, securityHeaders({ 'content-type': 'application/json', 'cache-control': 'no-store' }));
      res.end(JSON.stringify({ ...presence, registered, ciaUrl, gameUrl: gamePublicUrl }));
    } catch {
      res.writeHead(503, securityHeaders({ 'content-type': 'application/json', 'cache-control': 'no-store' }));
      res.end(JSON.stringify({ ok: false, error: 'presence_unavailable', ciaUrl, gameUrl: gamePublicUrl }));
    }
