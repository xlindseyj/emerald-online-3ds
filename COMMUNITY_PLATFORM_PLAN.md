...
- Gate 4: feasibility transport remains experimental in release 0.8.4. gpSP `mul_poke`, the libretro netpacket callbacks, bounded two-player relay rooms, strict opt-in configuration, hard-interruption reconnect, and newest-three timestamped save backups pass unit and two-instance Azahar tests. Earlier production-WSS relay testing delivered 53 bounded synthetic packets with reconnect at 182.9 ms average round trip and 5.6 ms mean absolute jitter. Checksum-valid progressed physical and Azahar saves pass the privacy-safe inspector; the exact v0.8.4 artifacts are installed and hash-verified on the Old 3DS, and a separately cloned v0.8.4 Azahar profile authenticates through production WSS using the same existing private room. The earlier v0.8.2 battle-overlay defect is closed by the retained overworld callback/battle guard and a physical v0.8.3 recheck. The v0.8.4 Azahar peer is currently hosting the recreated private room while the physical client awaits one reconnect. Real Cable Club battle, trade, save-integrity, interruption, loss-tolerance, current FPS, and audio checks remain open. The go/no-go decision has not been made and public invitations/rankings remain disabled.
