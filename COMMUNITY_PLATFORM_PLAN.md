...
- Gate 4: feasibility transport remains experimental in release 0.8.7. The libretro netpacket callbacks, bounded two-player relay rooms, strict opt-in configuration, hard-interruption reconnect, and newest-three timestamped save backups pass unit and two-instance Azahar tests. Version 0.8.7 selects gpSP's Emerald RFU backend, services its tighter packet windows, preserves transient host scans, and suppresses external presence overlays on native multiplayer maps. Two isolated Azahar 2126.0 clients discovered Emerald's native avatars and Trading Board offer, exchanged Torchic for Marill through the Union Room, completed the native animation and automatic save, restarted, and reloaded the exchanged parties. Checksum-valid progressed physical and Azahar saves pass the privacy-safe inspector, and the earlier v0.8.2 battle-overlay defect remains closed by the overworld callback/battle guard and physical v0.8.3 recheck. A physical 3DS-to-second-client Union Room battle and trade, interrupted-session recovery, save comparison, public-WSS latency/loss observation, version 0.8.7 FPS, and audio checks remain open. The go/no-go decision has not been made and public invitations/rankings remain disabled.
