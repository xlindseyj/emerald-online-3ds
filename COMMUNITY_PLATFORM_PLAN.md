...
- Gate 4: feasibility transport remains experimental in release 0.8.3. gpSP `mul_poke`, the libretro netpacket callbacks, bounded two-player relay rooms, strict opt-in configuration, hard-interruption reconnect, and newest-three timestamped save backups pass unit and two-instance Azahar tests. Earlier production-WSS relay testing delivered 53 bounded synthetic packets with reconnect at 182.9 ms average round trip and 5.6 ms mean absolute jitter. A checksum-valid progressed save passes the privacy-safe inspector, and the exact 0.8.3 3DSX loads it, authenticates through production WSS, enters the isolated link room, and remains stable in Azahar interpreter mode. A v0.8.2 physical run exposed stale remote-trainer overlays during a wild battle; v0.8.3 verifies Emerald's active overworld callback and battle flag, Azahar confirms visible-overworld, hidden-battle, restored-overworld behavior while connected, and the physical 3DS recheck with a same-map production peer confirmed the battle-hiding fix. The v0.8.3 artifacts are staged and hash-verified with rollback copies. The real Cable Club battle, trade, save-integrity, interruption, loss-tolerance, FPS, and audio checks remain open. The go/no-go decision has not been made and public invitations/rankings remain disabled.
