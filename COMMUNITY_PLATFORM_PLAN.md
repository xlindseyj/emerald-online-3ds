...
- Gate 4: feasibility transport remains experimental in release 0.8.2. gpSP `mul_poke`, the libretro netpacket callbacks, bounded two-player relay rooms, strict opt-in configuration, hard-interruption reconnect, and newest-three timestamped save backups pass unit and two-instance Azahar tests. Earlier production-WSS relay testing delivered 53 bounded synthetic packets with reconnect at 182.9 ms average round trip and 5.6 ms mean absolute jitter. A checksum-valid progressed save now passes the privacy-safe inspector, and the exact 0.8.2 3DSX loads it, authenticates through production WSS, enters the isolated link room, and remains stable in Azahar interpreter mode. The matching 0.8.2 3DSX, CIA, and shared-room configuration are staged and hash-verified on the physical console; FBI installation of the staged CIA and the real Cable Club battle, trade, save-integrity, interruption, loss-tolerance, FPS, and audio checks remain open. The go/no-go decision has not been made and public invitations/rankings remain disabled.
