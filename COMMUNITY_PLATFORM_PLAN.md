...
- Gate 4: feasibility transport remains experimental in release 0.8.0. gpSP `mul_poke`, the libretro netpacket callbacks, bounded two-player relay rooms, strict opt-in configuration, hard-interruption reconnect, and newest-three timestamped save backups pass unit and two-instance Azahar tests. The current 0.8.0 rerun passed four two-instance startup/interruption cycles and a fresh production-WSS relay delivered 53 bounded synthetic packets with reconnect at 182.9 ms average round trip and 5.6 ms mean absolute jitter. `npm run prepare:link-test` now creates a privacy-safe physical SD bundle and isolated Azahar profile for the real test. No progressed private save is present, so the emulators still emit zero Cable Club packets; battle, trade, save integrity, real loss tolerance, and physical performance acceptance remain open. The go/no-go decision has not been made and public invitations/rankings remain disabled.
