...
- Gate 0: complete and published to the private development mirror.
- Gate 1: implementation, automated/live acceptance, and physical Old 3DS XL WSS/SD identity acceptance are complete. The physical tests exposed and verified fixes for header capitalization and 3DS-local-clock certificate validation.
- Gate 2: implementation, PostgreSQL migration, public deployment, release packaging, automated/live acceptance, physical WSS, and browser pairing are complete.
- Gate 3: implementation, migrations, release packaging, production deployment, PostgreSQL lifecycle tests, live synthetic lifecycle, Azahar production-WSS acceptance, and physical bottom-screen/consent checks are complete. Public profiles and opt-in leaderboards remain intentionally privacy-limited, and battle/trade rankings remain disabled.
- Gate 4: feasibility transport remains experimental in release 0.8.9. The libretro netpacket callbacks, bounded two-player relay rooms, strict opt-in configuration, hard-interruption reconnect, and newest-three timestamped save backups pass unit and two-instance Azahar tests. The current runtime selects gpSP's Emerald RFU backend, services its tighter packet windows, preserves transient host scans, and suppresses external presence overlays on native multiplayer maps. Two isolated Azahar 2126.0 clients completed a Union Room trade through animation, save, restart, and exchanged-party verification. A later two-client battle reached the native LINZ-versus-CODEX screen on one peer while the other received Emerald's communication error after party exchange, so battle acceptance fails and remains open. Checksum-valid progressed physical and Azahar saves pass the privacy-safe inspector, and the earlier battle-overlay defect remains closed by the overworld callback/battle guard and physical recheck. Physical battle/trade, interrupted-session recovery, save comparison, public-WSS latency/loss observation, version 0.8.9 FPS, and audio checks remain open. The go/no-go decision has not been made and public invitations/rankings remain disabled.
